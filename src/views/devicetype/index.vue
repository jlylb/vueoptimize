  ...
  <div class="table-layout">
    ...
    >
      ...
      <template slot-scope="{ data }" slot="actionExtra">
        <el-button type="primary" size="mini" @click="openView(data,'realdata')">实时数据</el-button>
        <el-button type="primary" size="mini" @click="openView(data, 'historydata')">历史数据</el-button>
      </template>
    </table-list>
    <el-dialog
      :title="dialogTitle"
...
export default {
  ...
  methods: {
    ...
    openView(data, type) {
      const table =
        type === "realdata" ? data.dt_rtdata_table : data.dt_hisdata_table;
      this.$router.push({
        name: "api.data.index",
        params: { table: table, typeid: data.dt_typeid }
      });
    }
  },
  created() {
